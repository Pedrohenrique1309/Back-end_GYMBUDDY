...
//Função para inserir no Banco de Dados um novo exercicio_treino
const insertExercicioTreino = async function(exercicioTreino){
    ...
    try{
        ...
        if(result){
            let sqlSelectId = `SELECT * FROM tbl_exercicio_treino WHERE nome = '${exercicioTreino.id_treino}' ORDER BY id DESC LIMIT 1`
...
//Função para excluir no Banco de Dados um exercicio treino existente 
...
//Função para retornar do Banco de Dados uma lista de exercicio treino
...
//Função para buscar no Banco de Dados um exercicio treino pelo ID
